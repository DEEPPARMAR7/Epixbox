...
export default function AdminRazorpayPayButton({ order, onSuccess, onError }) {
  const handleClick = async () => {
    if (!order) return
    try {
      const payload = {
        amount_cents: Number(order.total_cents || 100),
        buyerEmail: order.buyer_email,
        buyerName: order.buyer_name || undefined,
      }

      const resp = await fetch(apiUrl('/debug-razorpay/create-order'), {
        method: 'POST',
